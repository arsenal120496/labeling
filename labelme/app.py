...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(
        ...
        if config['labels']:
            ...
            self.uniqLabelList.itemClicked.connect(self.labelsItemClick)
    ...
    def newShape(self):
        ...
        if items:
            for s in items:
                if s.text() == self.canvas.currentLabel:
                    text = s.text()
    ...
    def labelsItemClick(self, item):
        if str(item.text()) == 'Lane 1':
            color = QtGui.QColor(0, 255, 255)
            self.lineColor = color
            self.canvas.lineColor = color
            self.toggleDrawMode(False, createMode='linestrip')
            self.canvas.currentLabel = 'Lane 1'
        if str(item.text()) == 'Lane 2':
            color = QtGui.QColor(0, 0, 0)
            self.lineColor = color
            self.canvas.lineColor = color
            self.toggleDrawMode(False, createMode='linestrip')
            self.canvas.currentLabel = 'Lane 2'
        if str(item.text()) == 'Lane 3':
            color = QtGui.QColor(255, 255, 255)
            self.lineColor = color
            self.canvas.lineColor = color
            self.toggleDrawMode(False, createMode='linestrip')
            self.canvas.currentLabel = 'Lane 3'
